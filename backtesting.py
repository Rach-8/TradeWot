...
def rf_engineer_features(data_df,ema_short,ema_long,thresh):
    ...
    data["Vol_Z"] = (
        data["Vol_21"] - data["Vol_21"].rolling(252).mean()
    ) / data["Vol_21"].rolling(252).std()
    ...
    # ======================================================
    tickers = [
        "AAPL","MSFT","AMZN","GOOGL","META","NVDA","TSLA","JPM","UNH","XOM",
        "JNJ","V","PG","MA","HD","AVGO","LLY","MRK","PEP","ABBV",
        "COST","KO","CVX","WMT","BAC","AMD","DIS","CSCO","PFE","ORCL"
    ]
    ...
    "EMA_slow","EMA_fast",
    "volatility_dcw","volume_sma_em",
    "trend_adx",
    "volatility_bbh","trend_visual_ichimoku_a","volume_cmf",
    "trend_macd","volatility_bbm","volume_adi","volume_vpt","volume_obv","trend_stc",
    "momentum_ppo_hist","momentum_pvo_signal",
"volatility_kcw","trend_ichimoku_a",
    "momentum_pvo"
]
    ...
    base_features = [
        ...
        "ROC_5","BB_width",
        "EMA_Slope_Long","ADX_14",
        "Rates_Change","EMA_fast","EMA_slow"
    ]
...
def hmm_engineer_features(train_start,train_end,test_start,test_end,variance_target):

    spx     = yf.download("^GSPC", start="2000-01-01", auto_adjust=True)
    vix     = yf.download("^VIX",  start="2000-01-01", auto_adjust=True)
    vix3m   = yf.download("^VIX3M",start="2000-01-01", auto_adjust=True)
    vix6m   = yf.download("^VIX6M",start="2000-01-01", auto_adjust=True)
    russell= yf.download("^RUT",  start="2000-01-01", auto_adjust=True)
    hyg     = yf.download("HYG",  start="2000-01-01", auto_adjust=True)
    lqd     = yf.download("LQD",  start="2000-01-01", auto_adjust=True)
    ...
    # =========================
    exclude = {
        "date","spx_Close","rut_Close",
        "HYG","LQD",
        "SPX_50D_MA","SPX_200D_MA",
        "vix3m_Close","vix6m_Close"
    }

    feature_cols = [
        c for c in df.select_dtypes(np.number).columns
        if c not in exclude
    ]
    ...
    X_train_df = X_train_df.replace([np.inf,-np.inf],np.nan).dropna()
    X_test_df  = X_test_df.replace([np.inf,-np.inf],np.nan).dropna()
    ...
    return df,X_train_final,X_test_final,train_idx,test_idx

train_start = "2012-12-31"
...
spx = yf.download("SPY", start="2018-01-01",end="2023-12-15", auto_adjust=True)
if isinstance(spx.columns, pd.MultiIndex):
    spx.columns = spx.columns.get_level_values(0)

rf_features, rf_feature_cols = rf_engineer_features(spx,ema_short=20,ema_long=50,thresh=0.001)
rf_features = rf_features.dropna().reset_index(drop=True)



X = rf_features[rf_feature_cols]
y = rf_features["y_signal"]
...
step_days = 5 * retrain_every_weeks
test_end = len(X) 
...
test_index = rf_features.index[test_start:]
...
while start_idx < len(X_test_full):
    ...
    pos_signal[
        (y_prob > hi) |
        ((y_prob > 0.50) & (y_prob <= hi) & trend_long)
    ] = 1

    # -------- SHORT --------
    pos_signal[
        (y_prob < lo) |
        ((y_prob >= lo) & (y_prob < 0.50) & trend_short)
    ] = -1

    signals_list.append(
        pd.Series(pos_signal, index=test_index[start_idx:end_idx])
    )
...
close_series = rf_features["Close"].reindex(signals.index)
...
buy_hold_equity = rf_features["Close"].iloc[test_start:test_end] / rf_features["Close"].iloc[test_start] * 10000
...
fig.add_trace(go.Scatter(
    x=rf_features.index[test_start:test_end],
    y=equity,
    mode='lines',
